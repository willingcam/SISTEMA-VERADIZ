...
class Usuarios{
	...
	function login(){
		...
		// query to read single record
		$query = "SELECT p.id, p.nombre, p.correo, p.imagen, p.ubicacion_imagen, p.tipoUsuarioId, p.rol, p.rolID, p.contacto, p.activo FROM " . $this->table_name . "  p
		           WHERE  p.usuario = ? and p.claveacceso = ? AND activo = 1
				  LIMIT 0,1";
