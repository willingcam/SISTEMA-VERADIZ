...
(function() {
    ...
    app.controller("umaGetCtrl", ["$scope", "UmaService", 'DTOptionsBuilder', umaGetCtrl]);

    function umaGetCtrl($scope, UmaService, DTOptionsBuilder) {
        ...
        $scope.exportpdf = function() {
            ...
            docUMA.addImage(imageHeader, 'JPEG', 38, 10, 57, 57);

            docUMA.setFontSize(18);
            docUMA.setTextColor(40);

            docUMA.text("Unidad de medida y actualización", 280, 60);
            docUMA.text("VERA, DIZ Y CIA., S.C.", 300, 31);

            var listaResultados = [];
            for (var obj in $scope.salariosGet) {
                var item = [];
                item.push($scope.salariosGet[obj].anio);
                item.push($scope.salariosGet[obj].valor);

                listaResultados.push(item);
            }


            var columns = ["Año", "Valor"];
            docUMA.autoTable(columns, listaResultados, {
                theme: 'grid',
                startY: 90,
