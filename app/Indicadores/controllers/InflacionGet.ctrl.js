/*AYUDA:
FooEntitiesService nombre de factory en RolesGet.service.js
*/

(function() {
    "use strict";
    var app = angular.module("veradiz");
    app.controller("InflacionGetCtrl", ["$scope", "IndicesService", 'DTOptionsBuilder', InflacionGetCtrl]);

    function InflacionGetCtrl($scope, IndicesService, DTOptionsBuilder) {



        $scope.resultadosBusqueda = [];

        $scope.inflacion1 = [];
        $scope.inflacion2 = [];
        $scope.inflacion3 = [];
        $scope.inflacionacumulada1 = [];
        $scope.inflacionacumulada2 = [];
        $scope.inflacionacumulada3 = [];
        $scope.inflacionacumuladaanterior1 = [];
        $scope.inflacionacumuladaanterior2 = [];
        $scope.inflacionacumuladaanterior3 = [];

        $scope.registrofechas1 = [];
        $scope.registrofechas2 = [];
        $scope.registrofechas3 = [];




        $scope.fechai = new Date();
        $scope.fechat = new Date();

        $scope.fechaiC1 = new Date();
        $scope.fechatC1 = new Date();

        $scope.fechaiC2 = new Date();
        $scope.fechatC2 = new Date();

        $scope.fechaiC3 = new Date();
        $scope.fechatC3 = new Date();

        $scope.classPositiva = "0";

        $scope.fechai.setDate($scope.fechai.getDate() - 365);

        $scope.fechaiC1.setDate($scope.fechaiC1.getDate() - 365);
        $scope.fechaiC2.setDate($scope.fechaiC2.getDate() - 365);
        $scope.fechaiC3.setDate($scope.fechaiC3.getDate() - 365);

        $scope.mostrarGaficaResultados = false;
        $scope.mostrarGaficaComparativa = false;

        $scope.primerValor = "";
        $scope.primerDiferencia = "";
        $scope.anteriorValor = "";


        $scope.labels = [];
        $scope.series = ['Series A'];

        $scope.data = [];


        $scope.dtOptions = DTOptionsBuilder.newOptions().withButtons([{
                    extend: 'excelHtml5',
                    text: '<i class="fa fa-download"></i> Descargar Excel',
                    className: 'btn btn-success',
                    title: 'inflacion'
                },
                {
                    text: '<i class="fa fa-download"></i> Descargar PDF',
                    key: '1',
                    className: 'btn btn-success',
                    action: function(e, dt, node, config) {
                        $scope.exportpdf();
                    }
                },

                {
                    text: '<i class="fa fa-download"></i> Descargar gráfica PDF',
                    key: '1',
                    className: 'btn btn-success',
                    action: function(e, dt, node, config) {
                        $scope.exportGraficapdf();
                    }
                }

            ])
            .withDOM('lftr<"default"ip><"clear">B')
            .withOption('order', false)
            .withDisplayLength(10);



        $scope.reset = function() {
            $scope.fechai = new Date();
            $scope.fechat = new Date();


            $scope.fechaiC1 = new Date();
            $scope.fechatC1 = new Date();

            $scope.fechaiC2 = new Date();
            $scope.fechatC2 = new Date();

            $scope.fechaiC3 = new Date();
            $scope.fechatC3 = new Date();

            $scope.resultadosBusqueda = [];


            $scope.inflacion1 = [];
            $scope.inflacion2 = [];
            $scope.inflacion3 = [];
            $scope.inflacionacumulada1 = [];
            $scope.inflacionacumulada2 = [];
            $scope.inflacionacumulada3 = [];
            $scope.inflacionacumuladaanterior1 = [];
            $scope.inflacionacumuladaanterior2 = [];
            $scope.inflacionacumuladaanterior3 = [];

            $scope.registrofechas1 = [];
            $scope.registrofechas2 = [];
            $scope.registrofechas3 = [];

            $scope.registrofechas = [];

            $scope.ValidForm.$setPristine();
            $scope.$broadcast('angucomplete-alt:clearInput');
        };

        $scope.cambiafecha = function() {
            var fechaHoy = new Date();

            if ($scope.fechai > fechaHoy) {
                toastr.warning("La fecha de inicio no debe se mayor a la actual.", '', { timeOut: 10000 });
                $scope.fechai = new Date();
            }

            if ($scope.fechat > fechaHoy) {
                toastr.warning("La fecha de termino no debe se mayor a la actual.", '', { timeOut: 10000 });
                $scope.fechat = new Date();
            }

            if ($scope.fechai > $scope.fechat) {
                toastr.warning("La fecha de inicio no debe se mayor a la fecha de termino.", '', { timeOut: 10000 });
                $scope.fechai = new Date();
            }

        };

        $scope.update = function() {
            $scope.obtenerInformacion();
        }

        $scope.obtenerInformacion = function() {
            if ($scope.fechai != "" || $scope.fechat != "") {
                var diaInicio = $scope.fechai.getDate();
                var monthInicio = $scope.fechai.getMonth() + 1;
                var fechaInicioParametro = monthInicio + "/" + diaInicio + "/" + $scope.fechai.getFullYear();

                var diaTermino = $scope.fechat.getDate();
                var monthTermino = $scope.fechat.getMonth() + 1;
                var fechaTerminoParametro = monthTermino + "/" + diaTermino + "/" + $scope.fechat.getFullYear();


                var registro = {
                    "fechai": fechaInicioParametro,
                    "fechat": fechaTerminoParametro
                };


                IndicesService.getInflacionAcumulada(registro).then(
                    function(result) {

                        $scope.resultadosBusqueda = result.data.records;
                        $scope.primerValor = $scope.resultadosBusqueda[0].imensual;

                        $scope.anteriorValor = $scope.resultadosBusqueda[1].imensual;

                    },
                    function(err) {
                        toastr.error("No se han podido cargar la información solicitada");
                    }
                );

            } else {
                toastr.error("Debe seleccionar una fecha de inicio y de termino para establecer el rango de búsqueda");
            }
        }

        $scope.obtenerInformacion();


        $scope.exportpdf = function() {

            var imageHeader = "data:image/jpeg;base64,";
            var docINFLACION = new jsPDF('l', 'pt');

            var fechaHoy1 = new Date();
            var mesHoy1 = fechaHoy1.getMonth() + 1;

            var fechaImprime = mesHoy1 + "/" + fechaHoy1.getDate() + "/" + fechaHoy1.getFullYear();

            var header = function(data) {
                docINFLACION.setFontSize(20);
                docINFLACION.setTextColor(40);
                docINFLACION.setFontStyle('normal');
                docINFLACION.addImage(imageHeader, 'JPGE', data.settings.margin.left, 17, 57, 57);
                //doc.addImage(imageHeader, 'PNG', data.settings.margin.left - 8, 20, 150, 77);
                docINFLACION.text("Registro hisórico de la inflación", data.settings.margin.left + 250, 60);
                docINFLACION.text("VERA, DIZ Y CIA., S.C.", data.settings.margin.left + 270, 31);
            };
            var totalPagesExp = "{total_pages_count_string}";
            var footer = function(data) {
                var str = "Página " + data.pageCount;
                // Total page number plugin only available in jspdf v1.0+
                if (typeof docINFLACION.putTotalPages === 'function') {
                    str = str + " de " + totalPagesExp;
                }
                docINFLACION.text(str, data.settings.margin.left, docINFLACION.internal.pageSize.height - 30);
                docINFLACION.setFontSize(8);
                docINFLACION.text(fechaImprime, data.settings.margin.left + 660, docINFLACION.internal.pageSize.height - 30);
            };

            var options = {
                beforePageContent: header,
                afterPageContent: footer,
                margin: { top: 105 },
                startY: 130,
                styles: { cellPadding: 2, overflow: 'linebreak' },
                headerStyles: {
                    rowHeight: 15,
                    fontSize: 8,
                    textColor: 255,
                    fillColor: [115, 135, 156],
                    fontStyle: 'bold'
                },
                bodyStyles: { rowHeight: 12, fontSize: 8, valign: 'top', halign: 'center' },
                columnStyles: {
                    0: { columnWidth: 50 },
                    1: { columnWidth: 180, halign: 'left' },
                    2: { columnWidth: 180, halign: 'left' },
                    3: { columnWidth: 180, halign: 'left' },
                    4: { columnWidth: 180, halign: 'left' }
                    //4: { halign: 'left' }
                }
            };

            var res = docINFLACION.autoTableHtmlToJson(document.getElementById("inflacion"));
            docINFLACION.autoTable(res.columns, res.data, options);
            // Total page number plugin only available in jspdf v1.0+
            if (typeof docINFLACION.putTotalPages === 'function') {
                docINFLACION.putTotalPages(totalPagesExp);
            }

            docINFLACION.save("inflacion.pdf");

        }

        $scope.exportGraficapdf = function() {
            var newCanvas = document.querySelector('#myChart');

            //create image from dummy canvas
            var newCanvasImg = newCanvas.toDataURL("image/png", 1.0);

            //creates PDF from img
            var doc = new jsPDF('landscape');


            doc.setFontSize(20);
            var doc = new jsPDF('landscape');
            doc.setFontSize(20);

            doc.text(120, 15, "VERA, DIZ Y CIA., S.C.");


            doc.text(15, 30, "Gráfica de inflación");
            doc.addImage(newCanvasImg, 'PNG', 10, 40, 280, 150);
            doc.save('graficainflacion.pdf');
        }


        $scope.graficarResultados = function() {

            $scope.mostrarGaficaResultados = true;
            $scope.mostrarGaficaComparativa = false;

            if ($scope.fechai != "" || $scope.fechat != "") {
                var diaInicio = $scope.fechai.getDate();
                var monthInicio = $scope.fechai.getMonth() + 1;

                if (monthInicio < 10) {
                    monthInicio = "0" + monthInicio;
                }

                var fechaInicioParametro = monthInicio + "/" + diaInicio + "/" + $scope.fechai.getFullYear();

                var diaTermino = $scope.fechat.getDate();
                var monthTermino = $scope.fechat.getMonth() + 1;

                if (monthTermino < 10) {
                    monthTermino = "0" + monthTermino;
                }

                var fechaTerminoParametro = monthTermino + "/" + diaTermino + "/" + $scope.fechat.getFullYear();

                var registro = {
                    "fechai": fechaInicioParametro,
                    "fechat": fechaTerminoParametro
                };

                IndicesService.graficainflacion(registro).then(
                    function(result) {

                        var data = {};

                        var data = {
                            labels: result.data.fechas,
                            datasets: [{
                                    label: "Inflación anual",
                                    fill: false,
                                    lineTension: 0.1,
                                    backgroundColor: "#ff0000",
                                    borderColor: "#ff0000",
                                    borderCapStyle: 'butt',
                                    borderDash: [],
                                    borderDashOffset: 0.0,
                                    borderJoinStyle: 'miter',
                                    pointBorderColor: "#000000",
                                    pointBackgroundColor: "#90111A",
                                    pointBorderWidth: 1,
                                    pointHoverRadius: 2,
                                    pointHoverBackgroundColor: "#ff0000",
                                    pointHoverBorderColor: "#90111A",
                                    pointHoverBorderWidth: 3,
                                    pointRadius: 2,
                                    pointHitRadius: 2,
                                    data: result.data.iacumulada,
                                    spanGaps: true,
                                },
                                {
                                    label: "Inflación mensual",
                                    fill: false,
                                    lineTension: 0.1,
                                    backgroundColor: "#2626FF",
                                    borderColor: "#2626FF",
                                    borderCapStyle: 'butt',
                                    borderDash: [],
                                    borderDashOffset: 0.0,
                                    borderJoinStyle: 'miter',
                                    pointBorderColor: "#000000",
                                    pointBackgroundColor: "#90111A",
                                    pointBorderWidth: 1,
                                    pointHoverRadius: 2,
                                    pointHoverBackgroundColor: "#2626FF",
                                    pointHoverBorderColor: "#90111A",
                                    pointHoverBorderWidth: 3,
                                    pointRadius: 2,
                                    pointHitRadius: 2,
                                    data: result.data.imensual,
                                    spanGaps: true,
                                },
                                {
                                    label: "Inflación acumulada en el año",
                                    fill: false,
                                    lineTension: 0.1,
                                    backgroundColor: "#00B200",
                                    borderColor: "#00B200",
                                    borderCapStyle: 'butt',
                                    borderDash: [],
                                    borderDashOffset: 0.0,
                                    borderJoinStyle: 'miter',
                                    pointBorderColor: "#000000",
                                    pointBackgroundColor: "#90111A",
                                    pointBorderWidth: 1,
                                    pointHoverRadius: 2,
                                    pointHoverBackgroundColor: "#00B200",
                                    pointHoverBorderColor: "#90111A",
                                    pointHoverBorderWidth: 3,
                                    pointRadius: 2,
                                    pointHitRadius: 2,
                                    data: result.data.inflaciona,
                                    spanGaps: true,
                                }

                            ]
                        };
                        var ctx = document.getElementById("myChart");
                        $scope.myLineChart = new Chart(ctx, {
                            type: 'line',
                            data: data,
                            options: {
                                responsive: true,
                                scales: {
                                    xAxes: [{
                                        display: true,
                                        labelString: '%',
                                    }],
                                    yAxes: [{
                                        display: true,
                                        labelString: '%',
                                        ticks: {
                                            min: -2,
                                            stepSize: 1
                                        }
                                    }],
                                }
                            },

                        });
                    },
                    function(err) {
                        toastr.error("No se han podido cargar la información solicitada");
                    }
                );


            } else {
                toastr.error("Debe seleccionar una fecha de inicio y de termino para establecer el rango de búsqueda");
            }


            $scope.inflacion1 = [];
            $scope.inflacion2 = [];
            $scope.inflacion3 = [];
            $scope.inflacionacumulada1 = [];
            $scope.inflacionacumulada2 = [];
            $scope.inflacionacumulada3 = [];
            $scope.inflacionacumuladaanterior1 = [];
            $scope.inflacionacumuladaanterior2 = [];
            $scope.inflacionacumuladaanterior3 = [];

            $scope.registrofechas1 = [];
            $scope.registrofechas2 = [];
            $scope.registrofechas3 = [];

            $scope.registrofechas = [];
        }


        $scope.graficaComparativa = function() {
            $scope.mostrarGaficaResultados = false;
            $scope.mostrarGaficaComparativa = true;

            $scope.comparativo1();

            $scope.inflacion1 = [];
            $scope.inflacion2 = [];
            $scope.inflacion3 = [];
            $scope.inflacionacumulada1 = [];
            $scope.inflacionacumulada2 = [];
            $scope.inflacionacumulada3 = [];
            $scope.inflacionacumuladaanterior1 = [];
            $scope.inflacionacumuladaanterior2 = [];
            $scope.inflacionacumuladaanterior3 = [];

            $scope.registrofechas1 = [];
            $scope.registrofechas2 = [];
            $scope.registrofechas3 = [];

            $scope.registrofechas = [];

        }

        $scope.comparativo1 = function() {

            $scope.inflacion1 = [];
            $scope.registrofechas1 = [];
            $scope.inflacionacumulada1 = [];
            $scope.inflacionacumuladaanterior1 = [];

            if ($scope.fechaiC1 != "" || $scope.fechatC1 != "") {
                var diaInicio = $scope.fechaiC1.getDate();
                var monthInicio = $scope.fechaiC1.getMonth() + 1;

                if (monthInicio < 10) {
                    monthInicio = "0" + monthInicio;
                }

                var fechaInicioParametro = monthInicio + "/" + diaInicio + "/" + $scope.fechaiC1.getFullYear();

                var diaTermino = $scope.fechatC1.getDate();
                var monthTermino = $scope.fechatC1.getMonth() + 1;

                if (monthTermino < 10) {
                    monthTermino = "0" + monthTermino;
                }

                var fechaTerminoParametro = monthTermino + "/" + diaTermino + "/" + $scope.fechatC1.getFullYear();

                var registro = {
                    "fechai": fechaInicioParametro,
                    "fechat": fechaTerminoParametro
                };

                IndicesService.graficainflacion(registro).then(
                    function(result) {

                        $scope.registrofechas1 = result.data.fechas;

                        //INFLACION ANUAL
                        $scope.inflacion1 = result.data.iacumulada;
                        //INFLACION MENSUAL
                        $scope.inflacionacumulada1 = result.data.imensual;
                        //INFLACION ACUMULADA EN EL AÑO
                        $scope.inflacionacumuladaanterior1 = result.data.inflaciona;

                        $scope.comparativo2();
                    },
                    function(err) {
                        toastr.error("No se han podido cargar la información solicitada");
                    }
                );
            } else {
                toastr.error("Debe seleccionar una fecha de inicio y de termino para establecer el rango de búsqueda");
            }
        }


        $scope.comparativo2 = function() {

            $scope.registrofechas2 = [];

            $scope.inflacion2 = [];
            $scope.inflacionacumulada2 = [];
            $scope.inflacionacumuladaanterior2 = [];

            if ($scope.fechaiC2 != "" || $scope.fechatC2 != "") {
                var diaInicio = $scope.fechaiC2.getDate();
                var monthInicio = $scope.fechaiC2.getMonth() + 1;

                if (monthInicio < 10) {
                    monthInicio = "0" + monthInicio;
                }

                var fechaInicioParametro = monthInicio + "/" + diaInicio + "/" + $scope.fechaiC2.getFullYear();

                var diaTermino = $scope.fechatC2.getDate();
                var monthTermino = $scope.fechatC2.getMonth() + 1;

                if (monthTermino < 10) {
                    monthTermino = "0" + monthTermino;
                }

                var fechaTerminoParametro = monthTermino + "/" + diaTermino + "/" + $scope.fechatC2.getFullYear();

                var registro = {
                    "fechai": fechaInicioParametro,
                    "fechat": fechaTerminoParametro
                };

                IndicesService.graficainflacion(registro).then(
                    function(result) {

                        $scope.registrofechas2 = result.data.fechas;

                        //INFLACION ANUAL
                        $scope.inflacion2 = result.data.iacumulada;
                        //INFLACION MENSUAL
                        $scope.inflacionacumulada2 = result.data.imensual;
                        //INFLACION ACUMULADA EN EL AÑO
                        $scope.inflacionacumuladaanterior2 = result.data.inflaciona;

                        $scope.comparativo3();
                    },
                    function(err) {
                        toastr.error("No se han podido cargar la información solicitada");
                    }
                );
            } else {
                toastr.error("Debe seleccionar una fecha de inicio y de termino para establecer el rango de búsqueda");
            }
        }

        $scope.comparativo3 = function() {

            $scope.registrofechas3 = [];
            $scope.inflacion3 = [];
            $scope.inflacionacumulada3 = [];
            $scope.inflacionacumuladaanterior3 = [];

            if ($scope.fechaiC3 != "" || $scope.fechatC3 != "") {
                var diaInicio = $scope.fechaiC3.getDate();
                var monthInicio = $scope.fechaiC3.getMonth() + 1;

                if (monthInicio < 10) {
                    monthInicio = "0" + monthInicio;
                }

                var fechaInicioParametro = monthInicio + "/" + diaInicio + "/" + $scope.fechaiC3.getFullYear();

                var diaTermino = $scope.fechatC3.getDate();
                var monthTermino = $scope.fechatC3.getMonth() + 1;

                if (monthTermino < 10) {
                    monthTermino = "0" + monthTermino;
                }

                var fechaTerminoParametro = monthTermino + "/" + diaTermino + "/" + $scope.fechatC3.getFullYear();

                var registro = {
                    "fechai": fechaInicioParametro,
                    "fechat": fechaTerminoParametro
                };

                IndicesService.graficainflacion(registro).then(
                    function(result) {

                        $scope.registrofechas3 = result.data.fechas;

                        //INFLACION ANUAL
                        $scope.inflacion3 = result.data.iacumulada;
                        //INFLACION MENSUAL
                        $scope.inflacionacumulada3 = result.data.imensual;
                        //INFLACION ACUMULADA EN EL AÑO
                        $scope.inflacionacumuladaanterior3 = result.data.inflaciona;

                        $scope.generaComparativa1();
                        $scope.generaComparativa2();
                        $scope.generaComparativa3();
                    },
                    function(err) {
                        toastr.error("No se han podido cargar la información solicitada");
                    }
                );
            } else {
                toastr.error("Debe seleccionar una fecha de inicio y de termino para establecer el rango de búsqueda");
            }
        }

        $scope.generaComparativa1 = function() {

            var data = {};

            var data = {
                labels: $scope.registrofechas1,
                datasets: [{
                        label: "Inflacion anual respecto al primer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#ff0000",
                        borderColor: "#ff0000",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#ff0000",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacion1,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación anual respecto al segundo rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#2626FF",
                        borderColor: "#2626FF",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#2626FF",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacion2,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación anual respecto al tercer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#00B200",
                        borderColor: "#00B200",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#00B200",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacion3,
                        spanGaps: true,
                    }

                ]
            };
            var ctx = document.getElementById("myChart2");
            $scope.myLineChart = new Chart(ctx, {
                type: 'line',
                data: data,
                options: {
                    responsive: true,
                    scales: {
                        xAxes: [{
                            display: true,
                            labelString: '%',
                        }],
                        yAxes: [{
                            display: true,
                            labelString: '%',
                            ticks: {
                                min: -2,
                                stepSize: 1
                            }
                        }],
                    }
                },

            });
        }


        $scope.generaComparativa2 = function() {

            var data = {};

            var data = {
                labels: $scope.registrofechas2,
                datasets: [{
                        label: "Inflacion mensual respecto al primer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#ff0000",
                        borderColor: "#ff0000",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#ff0000",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumulada1,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación mensual respecto al segundo rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#2626FF",
                        borderColor: "#2626FF",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#2626FF",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumulada2,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación mensual respecto al tercer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#00B200",
                        borderColor: "#00B200",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#00B200",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumulada3,
                        spanGaps: true,
                    }

                ]
            };
            var ctx = document.getElementById("myChart3");
            $scope.myLineChart = new Chart(ctx, {
                type: 'line',
                data: data,
                options: {
                    responsive: true,
                    scales: {
                        xAxes: [{
                            display: true,
                            labelString: '%',
                        }],
                        yAxes: [{
                            display: true,
                            labelString: '%',
                            ticks: {
                                min: -2,
                                stepSize: 1
                            }
                        }],
                    }
                },

            });
        }

        $scope.generaComparativa3 = function() {

            var data = {};

            var data = {
                labels: $scope.registrofechas3,
                datasets: [{
                        label: "Inflacion acumulada en el año con respecto al primer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#ff0000",
                        borderColor: "#ff0000",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#ff0000",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumuladaanterior1,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación acumulada en el año respecto al segundo rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#2626FF",
                        borderColor: "#2626FF",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#2626FF",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumuladaanterior2,
                        spanGaps: true,
                    },
                    {
                        label: "Inflación acumulada en el año con respecto al tercer rango de fechas",
                        fill: false,
                        lineTension: 0.1,
                        backgroundColor: "#00B200",
                        borderColor: "#00B200",
                        borderCapStyle: 'butt',
                        borderDash: [],
                        borderDashOffset: 0.0,
                        borderJoinStyle: 'miter',
                        pointBorderColor: "#000000",
                        pointBackgroundColor: "#90111A",
                        pointBorderWidth: 1,
                        pointHoverRadius: 2,
                        pointHoverBackgroundColor: "#00B200",
                        pointHoverBorderColor: "#90111A",
                        pointHoverBorderWidth: 3,
                        pointRadius: 2,
                        pointHitRadius: 2,
                        data: $scope.inflacionacumuladaanterior3,
                        spanGaps: true,
                    }

                ]
            };
            var ctx = document.getElementById("myChart4");
            $scope.myLineChart = new Chart(ctx, {
                type: 'line',
                data: data,
                options: {
                    responsive: true,
                    scales: {
                        xAxes: [{
                            display: true,
                            labelString: '%',
                        }],
                        yAxes: [{
                            display: true,
                            labelString: '%',
                            ticks: {
                                min: -2,
                                stepSize: 1
                            }
                        }],
                    }
                },

            });
        }



    }
})();