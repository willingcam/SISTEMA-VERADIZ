...
(function() {
    ...
    app.controller("UdisGetCtrl", ["$scope", "IndicesService", 'DTOptionsBuilder', UdisGetCtrl]);

    function UdisGetCtrl($scope, IndicesService, DTOptionsBuilder) {
        ...
        $scope.obtenerInformacion = function() {
            if ($scope.fechai != "" || $scope.fechat != "") {
                ...
                IndicesService.getUdis(registro).then(
                    function(result) {
        ...
        $scope.exportpdf = function() {
            ...
            var header = function(data) {
                docUDIS.setFontSize(20);
                docUDIS.setTextColor(40);
                docUDIS.setFontStyle('normal');
                docUDIS.addImage(imageHeader, 'JPGE', data.settings.margin.left, 17, 57, 57);
                //doc.addImage(imageHeader, 'PNG', data.settings.margin.left - 8, 20, 150, 77);
                docUDIS.text("UDIS", data.settings.margin.left + 340, 60);
                docUDIS.text("VERA, DIZ Y CIA., S.C.", data.settings.margin.left + 270, 31);
            };
            var totalPagesExp = "{total_pages_count_string}";
            var footer = function(data) {
                var str = "Página " + data.pageCount;
                // Total page number plugin only available in jspdf v1.0+
                if (typeof docUDIS.putTotalPages === 'function') {
                    str = str + " de " + totalPagesExp;
                }
                docUDIS.text(str, data.settings.margin.left, docUDIS.internal.pageSize.height - 30);
                docUDIS.setFontSize(8);
                docUDIS.text(fechaImprime, data.settings.margin.left + 660, docUDIS.internal.pageSize.height - 30);
            };
            ...
            var res = docUDIS.autoTableHtmlToJson(document.getElementById("udis"));
            docUDIS.autoTable(res.columns, res.data, options);
            // Total page number plugin only available in jspdf v1.0+
            if (typeof docUDIS.putTotalPages === 'function') {
                docUDIS.putTotalPages(totalPagesExp);
            }

            docUDIS.save("udis.pdf");
