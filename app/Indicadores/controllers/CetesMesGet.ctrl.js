    ...
    function CetesMesGetCtrl($scope, IndicesService, DTOptionsBuilder) {
        ...
        $scope.exportpdf = function() {
            ...
            var header = function(data) {
                docCetes.setFontSize(20);
                docCetes.setTextColor(40);
                docCetes.setFontStyle('normal');
                docCetes.addImage(imageHeader, 'JPGE', data.settings.margin.left, 17, 57, 57);
                //doc.addImage(imageHeader, 'PNG', data.settings.margin.left - 8, 20, 150, 77);

                if ($scope.tipo == "1") {
                    docCetes.text("Cetes 28 días", data.settings.margin.left + 320, 60);
                } else {
                    docCetes.text("Tasa objetivo", data.settings.margin.left + 320, 60);
                }

                docCetes.text("VERA, DIZ Y CIA., S.C.", data.settings.margin.left + 270, 31);
            };
            var totalPagesExp = "{total_pages_count_string}";
            var footer = function(data) {
                var str = "Página " + data.pageCount;
                // Total page number plugin only available in jspdf v1.0+
                if (typeof docCetes.putTotalPages === 'function') {
                    str = str + " de " + totalPagesExp;
                }
                docCetes.text(str, data.settings.margin.left, docCetes.internal.pageSize.height - 30);
                docCetes.setFontSize(8);
                docCetes.text(fechaImprime, data.settings.margin.left + 660, docCetes.internal.pageSize.height - 30);
            };
            ...
            var options = {
                ...
                columnStyles: {
                    ...
                    1: { columnWidth: 230, halign: 'left' },
                    2: { columnWidth: 230, halign: 'left' },
                    3: { columnWidth: 230,halign: 'center' }
                    //4: { halign: 'left' }
                }
            };

            var res = docCetes.autoTableHtmlToJson(document.getElementById("cetes"));
            docCetes.autoTable(res.columns, res.data, options);
            // Total page number plugin only available in jspdf v1.0+
            if (typeof docCetes.putTotalPages === 'function') {
                docCetes.putTotalPages(totalPagesExp);
            }

            docCetes.save("cetes.pdf");
        ...
        $scope.exportGraficapdf = function() {
            ...
            if ($scope.tipo == "1") {
                doc.text(15, 30, "Gráfica del valor de Cetes a 28 días");
            } else {
                doc.text(15, 30, "Gráfica del valor de la tasa objetivo");
            }


            doc.addImage(newCanvasImg, 'PNG', 10, 40, 280, 150);
            doc.save('graficacetes.pdf');
        }
