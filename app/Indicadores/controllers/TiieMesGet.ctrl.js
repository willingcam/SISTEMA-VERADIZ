...
(function() {
    ...
    app.controller("TiieMesGetCtrl", ["$scope", "IndicesService", 'DTOptionsBuilder', TiieMesGetCtrl]);

    function TiieMesGetCtrl($scope, IndicesService, DTOptionsBuilder) {
        ...
        $scope.obtenerInformacion = function() {
            if ($scope.fechai != "" || $scope.fechat != "") {
                ...
                if ($scope.tipo == "1") {
                    ...
                    IndicesService.graficaTIIE28(registro).then(
                        function(result) {
                    ...
                    if ($scope.tipo == "2") {
                        ...
                        IndicesService.graficaTIIE91(registro).then(
                            function(result) {
        ...
        $scope.exportpdf = function() {
            ...
            var header = function(data) {
                docTIIE.setFontSize(20);
                docTIIE.setTextColor(40);
                docTIIE.setFontStyle('normal');
                docTIIE.addImage(imageHeader, 'JPGE', data.settings.margin.left, 17, 57, 57);
                //doc.addImage(imageHeader, 'PNG', data.settings.margin.left - 8, 20, 150, 77);

                if ($scope.tipo == "1") {
                    docTIIE.text("TIIE 28 días", data.settings.margin.left + 320, 60);
                } else {
                    docTIIE.text("TIIE 91 días", data.settings.margin.left + 320, 60);
                }


                docTIIE.text("VERA, DIZ Y CIA., S.C.", data.settings.margin.left + 270, 31);
            };
            var totalPagesExp = "{total_pages_count_string}";
            var footer = function(data) {
                var str = "Página " + data.pageCount;
                // Total page number plugin only available in jspdf v1.0+
                if (typeof docTIIE.putTotalPages === 'function') {
                    str = str + " de " + totalPagesExp;
                }
                docTIIE.text(str, data.settings.margin.left, docTIIE.internal.pageSize.height - 30);
                docTIIE.setFontSize(8);
                docTIIE.text(fechaImprime, data.settings.margin.left + 660, docTIIE.internal.pageSize.height - 30);
            };
            ...
            var res = docTIIE.autoTableHtmlToJson(document.getElementById("tiie"));
            docTIIE.autoTable(res.columns, res.data, options);
            // Total page number plugin only available in jspdf v1.0+
            if (typeof docTIIE.putTotalPages === 'function') {
                docTIIE.putTotalPages(totalPagesExp);
            }

            docTIIE.save("tiie.pdf");
        ...
        $scope.exportGraficapdf = function() {
            ...
            if ($scope.tipo == "1") {
                doc.text(15, 30, "Gráfica del valor de TIIE a 28 días");
            } else {
                doc.text(15, 30, "Gráfica del valor de TIIE a 91 días");
            }

            doc.addImage(newCanvasImg, 'PNG', 10, 40, 280, 150);
            doc.save('graficatiie.pdf');
        }
