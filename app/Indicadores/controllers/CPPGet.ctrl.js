...
(function() {
    ...
    function CPPGetCtrl($scope, IndicesService, DTOptionsBuilder, AuthService) {
        ...
        $scope.delete = function(obj) {
            ...
            IndicesService.deleteCPP(obj).then(
                function(result) {
                    toastr.success("valor eliminado exitosamente");
                    $scope.obtenerInformacion();
                },
                function(err) {
                    toastr.error("Se presento un problema en la eliminación del valor solicitado");
                }
            );
        ...
        $scope.exportpdf = function() {
            ...
            docCPP.addImage(imageHeader, 'JPEG', 38, 10, 57, 57);

            docCPP.setFontSize(18);
            docCPP.setTextColor(40);

            docCPP.text("Costo porcentual promedio de captación", 280, 60);
            docCPP.text("VERA, DIZ Y CIA., S.C.", 300, 31);

            var listaResultados = [];
            for (var obj in $scope.resultadosBusqueda) {
                var item = [];
                item.push($scope.resultadosBusqueda[obj].fecha);
                item.push($scope.resultadosBusqueda[obj].valor);

                listaResultados.push(item);
            }


            var columns = ["Fecha", "Valor"];
            docCPP.autoTable(columns, listaResultados, {
                theme: 'grid',
                startY: 90,
