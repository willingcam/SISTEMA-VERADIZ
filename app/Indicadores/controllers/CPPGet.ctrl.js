...
(function() {
    ...
    app.controller("CPPGetCtrl", ["$scope", "IndicesService", 'DTOptionsBuilder', 'AuthService', CPPGetCtrl]);

    function CPPGetCtrl($scope, IndicesService, DTOptionsBuilder, AuthService) {
        ...
        $scope.rol = AuthService.authentication.idrol;

        $scope.fechai.setDate($scope.fechai.getDate() - 360);
        ...
        $scope.obtenerInformacion = function() {
            if ($scope.fechai != "" || $scope.fechat != "") {
                ...
                IndicesService.getCPP(registro).then(
                    function(result) {
